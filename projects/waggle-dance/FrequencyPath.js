...
d3.select("#frequencySelect").append("input")
    ...
    .on("mouseup", function input() {
	redrawPath(this.value);
	transition_frequency();
    });
...
function redrawPath(frequencyValue) {
    var pathToUse = points_mid;
    if (frequencyValue < 75) {
	pathToUse = points_min;
    } else if (frequencyValue > 125 ) {
	pathToUse = points_max;
    } else {
	pathToUse = points_mid;
    }
    
    d3.select(".dynamic_waggle").remove();
    svg_frequency = d3.select("#frequency").append("svg")
	.attr("class", "dynamic_waggle")
	.attr("id", "waggle_path")
	.attr("width", 350 + 50)
	.attr("height", 300 + 50).attr("transform", "translate(44,60) rotate(" + 42 +  ")");

    path_frequency = svg_frequency.append("path")
	.data([pathToUse])
	.attr("class", "dynamic_freq")
	.attr("id", "points")
	.attr("stroke-dasharray","5, 26")
    ...
    marker_frequency = svg_frequency.append("path")
	.attr("id", "frequencyBiene")
	.attr("class", "biene")
	.data([bee_points_frequency])
	.attr("d", d3.line().curve(d3.curveBasisClosed));
